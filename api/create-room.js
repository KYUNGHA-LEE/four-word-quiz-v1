// 방 생성 + hostUid 최초 등록 (서버에서만 처리)
// 강사 진입은 비밀번호가 아니라 주소 뒤 ?mode=admin 링크로 한다.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!idToken) return res.status(400).json({ error: "필수 값이 없습니다." });

    // 1) 강사 uid 확인 (토큰 검증 → 위조 방지)

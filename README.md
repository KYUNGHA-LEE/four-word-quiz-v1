...
Realtime Database → 데이터 탭에서 아래 구조로 문제를 넣습니다.
정답은 `answers` 에 여러 개 넣으면 그중 아무거나 맞으면 정답 처리됩니다.
(공백은 무시하고 비교합니다. 예: `김치 찌개` = `김치찌개`)
...
```json
{
  "questionBank": {
    "fourWordQuiz": {
      ...
      "questions": {
        "0": {
          "clue": "돼지고기와 김치를 넣고 끓인, 대표적인 한국 찌개는?",
          "answers": ["김치찌개", "김치찌게", "찌개"]
        },
        "1": {
          "clue": "버스를 운전하는 사람을 뜻하는 네 글자 말은? (버스와 관련된 말)",
          "answers": ["버스기사", "버스요금", "버스카드", "버스노선"]
        },
        "2": {
          "clue": "학교에서 공부를 가르쳐 주시는 분을 높여 부르는 말은?",
          "answers": ["선생님", "학교선생", "담임선생"]
        }
      }
    }
  }
